...
public class ResumenPedidoEnvV2 extends AppCompatActivity  {

    Bundle Extras;
    SDTProductos[] SDTProductos ;
    ListViewAdapterProductosEnvv2 ListViewAdapterProductos;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Btn_Reprocesar = (Button)findViewById(R.id.Btn_Reprocesar);
...
*/
        ...
        String xEmpresa=vGlobalVariables.getEmpresa();
        xEmpresa=xEmpresa.toUpperCase();
        Btn_Reprocesar.setVisibility(View.GONE);
        if(xEmpresa.equalsIgnoreCase("SUHOGAR") || xEmpresa.equalsIgnoreCase("SUHOGARPRU")){
            Btn_Reprocesar.setVisibility(View.VISIBLE);
        }
...
try {
    ...
    String Consulta = "select * from(select a.ArtSec,ArtCod,ArtNom, precio,Desc1,Desc2," + Exist + " Exist,artemb,case  cliiva when 'S' then ParConIva else 0 end ParConIva,ArtValImp,cant,InvGruNom,artmednomcom,KarUni,KarPrePub,cantcaj,ifNULL(pordesc,Desc1) pordesc,ifNULL(pordesc2,Desc2) pordesc2,ifNULL(pordesc3,0.0) pordesc3,ifNULL(pordesc4,0.0) pordesc4,ifNULL(pordesc5,0.0) pordesc5,ifNULL(pordesc6,0.0) pordesc6,confemp,confprov,confvend,p.bodcod,ifNULL(p.plazo,0) plazo,ifNULL(p.ConPagnom,'') plazonom,0 MovParPremSec,pedlisprecod from Articulos a " +
            ...
            "left join pedido p on prefijo='" + prefijo + "' and p.nitsec='" + nitsec + "' and p.clisec=" + clisec + " and p.artsec=a.artsec where cant+cantcaj<>0 and pdyear=" + time.year + " and pdmonth=" + (time.month + 1) + " and pdday=" + time.monthDay + " " +
            " union "+
            "select m.MovParPremArtSec,ArtCod,ArtNom,1 precio,0.0,0.0,0 Exist, a.artemb,0 ParConIva,0 ArtValImp,MovParPremCant cant,'' InvGruNom,'' artmednomcom,0 KarUni,0 KarPrePub,ifNULL(MovParPremCantCaj,0) cantcaj,0.0 pordesc,0.0 pordesc2,0.0 pordesc3,0.0 pordesc4,0.0 pordesc5,0.0 pordesc6,0.0 confemp,0.0 confprov,0.0 confvend,0 bodcod,0 plazo,'' plazonom,MovParPremSec,0 pedlisprecod   from MovParPrem M " +
            "left join articulos a on a.artsec=m.MovParPremArtSec where Prefijo='"+prefijo+"' and MovParNitSec='"+nitsec+"' and MovParCliSec="+clisec+" and MovParPremAno=" + time.year + " and MovParPremMes=" + (time.month + 1) + " and MovParPremDia=" + time.monthDay+") jj order by artcod,precio desc,cantcaj,cant ";
    ...
    if (cursor.getCount() > 0) {
        ...
        do {
            ...
            SDTProductosItem.LisPreCod = cursor.getInt(29); //Extras.getInt("lisprecod");
           ...
           /* try {
                try {
                    ...
                    Log.e("ArtSec: ",cursor.getString(0));
                    Log.e("sExistencia: ",String.valueOf(sExistencia));
    ...
    ListViewAdapterProductos = new ListViewAdapterProductosEnvv2(this, SDTProductos);
    ...
    Btn_Reprocesar.setOnClickListener(new View.OnClickListener() {
        @Override
        public void onClick(View view) {
                for (SDTProductos producto : SDTProductos) {
                    producto.recalcularPrecios();
                }

            listview_productos= (ListView) findViewById(R.id.listview_productos);
            ListViewAdapterProductos = new ListViewAdapterProductosEnvv2(ResumenPedidoEnvV2.this, SDTProductos);
            listview_productos.setAdapter(ListViewAdapterProductos);
        }
    });
        ...
        int hh=1;
        Log.e("Errror vcargarrr",e.toString());
    }
